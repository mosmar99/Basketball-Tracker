...
def align_frames_sift_cylindrical_homography(frames, ignore_masks=None, border=50):
...
def create_clean_background_torch(frames, device=None, chunk_size=64):
    import torch
    ...
    for y0 in tqdm(range(0, H, chunk_size), desc="[INFO] Background median (GPU chunks)"):
        ...
        chunk = torch.from_numpy(stack[:, y0:y1]).permute(0, 3, 1, 2).to(device)  # (N,3,h,W)
...
if __name__ == "__main__":
    ...
    background = create_clean_background_torch(aligned_no_players)
