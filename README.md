...
This project aims to apply computer vision techniques to basketball games. Similarly to baseball, basketball is both dynamic and continuous in nature. Applying the task of computer vision to this field can prove quite rewarding, offering rich and complex data to infer analytics from. Currently, the NBA utilizes statistics gathered by SportVU, which is a camera system that collects and provides real-time statistics such as player and ball positioning. The problem with this system is that it is both expensive and proprietary.

More specifically, we aim to automatically detect player and ball movements and project them onto a 2d map of the court. As well as utilizing the data gathered from this projection to create statistics for the game. Through this projection, one can infer the speed and positioning of both the players as well as the ball.
