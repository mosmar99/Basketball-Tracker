...
from utils.bbox_utils import get_center_bbox
...
def align_frames_sift_cylindrical_homography(frames, ignore_masks=None, border=1):
    sift = cv2.SIFT_create()
    h, w = frames[0].shape[:2]

    focal_length = w * 1.4
    cylindrical_frames = [_warp_to_cylinder(f, focal_length) for f in tqdm(frames, desc="[INFO] Warping frames to cylinder")]

    # --- Warp masks to cylindrical space if provided ---
    cylindrical_masks = None
    if ignore_masks is not None:
        cylindrical_masks = [_warp_to_cylinder(m, focal_length) for m in ignore_masks]
...
def draw_bbox_mask(frame, bbox, color):
    x1, y1, x2, y2 = map(int, bbox)
    cv2.rectangle(frame, (x1, y1), (x2, y2), color, thickness=cv2.FILLED)
    
    return frame

def mask_from_track(track, frame):
...
if __name__ == "__main__":
    ...
    frames = load_frames(video_path, sample_rate=1)
    ...
    masks = [temporal_mask | temporal_mask for mask in masks]

    aligned, aligned_masks, _, _ = align_frames_sift_cylindrical_homography(frames, ignore_masks=masks)

    aligned_no_players = []
    for frame, mask in zip(aligned, aligned_masks):
        masked = frame.copy()
        masked[mask > 0] = 0
        aligned_no_players.append(masked)

    background = create_clean_background(aligned_no_players)
    
    cv2.imshow("Player-Free Court (Cylindrical Projection - Homography)", background)
    cv2.imwrite("player_free_background_homography.jpg", background)
